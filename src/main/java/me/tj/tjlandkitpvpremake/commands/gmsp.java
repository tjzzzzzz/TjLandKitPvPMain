            ...
            if (isPlayerInStaffGroup(player)) {
                if (args.length == 0) {
                    if (player.getGameMode().equals(GameMode.SPECTATOR)) {
                        player.sendMessage(MiniMessage.miniMessage().deserialize("<bold><Aqua>Tj<white>Land</bold> You're already in spectator mode."));
                    } else {
                        player.setGameMode(GameMode.SPECTATOR);
                        player.sendMessage(MiniMessage.miniMessage().deserialize("<bold><Aqua>Tj<white>Land</bold> Gamemode set to spectator"));
                    }
                } else {
                    ...
                    if (target.getGameMode().equals(GameMode.SPECTATOR)) {
                        player.sendMessage(MiniMessage.miniMessage().deserialize("<bold><Aqua>Tj<white>Land</bold> " + targetname + " is already in spectator mode."));
                    } else {
                        player.sendMessage(MiniMessage.miniMessage().deserialize("<bold><Aqua>Tj<white>Land</bold> Gamemode spectator set for " + targetname));
